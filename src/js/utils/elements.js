export const elements = {
    storeList: () => document.querySelector('.store-list'),
    controls: () => document.querySelector('.controls'),
    leftControl: () => document.querySelector('.left__control'),
    addSpecControl: () => document.querySelector('.add-spec__control'),
    addSpecButton: () => document.querySelector('.add-spec__button'),
    avatarSpecList: () => document.querySelector('.spec-avatar__list'),
    avatarSpecSelected: () => document.querySelector('.selected'),
    avatarSpecSpacer: () => document.querySelector('.spec-avatar__spacer'),
    colSelectSpec: () => document.querySelector('.left'),
    storeListCountNumber: () => document.querySelector('.store-list__count-number'),
    selectSpecInput: () => document.querySelector('.select-spec__input'),
    selectSpecNameList: () => document.querySelector('.select-spec__namelist'),
    selectSpecClose: () => document.querySelector('.select-spec__close'),
    specConfigTrash: () => document.querySelector('.spec-config__trash')
};